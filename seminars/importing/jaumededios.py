...
def import_talks(csv_file):
    ...
    with open(csv_file) as F:
        for i, line in enumerate(reader(F)):
            if i == 0:
                assert line == ['Timestamp', 'Title', 'Speaker', 'Speaker_inst', 'Abstract', 'Host', 'Seminar', 'Site', 'In_Charge', 'arXiv', 'Date', 'Start_Time', 'End_Time', 'Timezone', 'Approved']
                continue
            ...
            if curtalk is not None:
                print("Talk with speaker %s already exists in seminar %s; continuing" % (speaker, seminar_id))
                continue
