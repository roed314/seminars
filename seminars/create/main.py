    ...
    for i in range(10):
        ...
        if D.get("homepage") or D.get("email") or D.get("full_name"):
            ...
            if not errmsgs and D["display"] and D["email"] and not D["homepage"]:
                flash_warning(format_errmsg("The email address %s of organizer %s will be publicily visible.<br>%s",
                               D["email"],D["full_name"],"Set homepage or disable display to prevent this."))
